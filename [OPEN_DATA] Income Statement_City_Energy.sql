...
#### DRAFT: Some changes will need to be rewritten for further analysis.
...
WITH income_statement_overall AS (
	SELECT t.company_name AS "Company Name",
           ...
           -- NOTE: Revenue is how much they are making from their sources (like service and resource assets).
		   CONCAT("$", ROUND(t.Income_Statement +  t.Expense), ".00") AS "Total Revenue",
		   -- Total Expentitudes (Expense)
		   CONCAT("$", ROUND(t.Expense), ".00") AS "Total Expense", 
           -- Income Statement (aka Budget) 
		   -- NOTE: Determines if they have enough money to be spend on following resources used 
           -- NOTE: Budget determines how much money you have left after spending to determine the next financial goal.
		   -- Subtract from the following to determine net profit or income deflict 
		   CONCAT("$", ROUND(t.Income_Statement), ".00") AS "Income Statement", 
		   -- Check Net Income or Income Defict by making boolean statemnt
           IF ((t.Income_Statement) > 0, "Profit", "Deficit") AS "Profit or Loss"
	...
	FROM (SELECT Department_Name AS company_name,
				 ...
				 SUM(Budget) AS Income_Statement,
...
WITH income_statement_company_unit AS (
	...
	FROM (SELECT Department_Name AS company_name,
		   ...
		   FROM Budget_Timeline BT
			...
			HAVING Fiscal_Year = '2026' AND Fiscal_Quarter = '2' AND Department_Name = 'Austin Energy')t
	-- Order the following
	ORDER BY FY ASC, FQ ASC, unit ASC)
...
WITH income_statement_expense_type_overall AS (
	SELECT t.company_name AS "Company Name",
		   t.FY AS "FY",
		   t.FQ AS "Quarter",
           t.exp_name AS "By Each Company-Related Expenses", 
		   -- Total Revenue
		   -- Subtract from the following to determine net profit or income deflict 
           -- NOTE: Revenue is how much they are making from their sources (like service and resource assets).
		   CONCAT("$", ROUND(t.Income_Statement +  t.Expense), ".00") AS "Total Revenue",
	...
	FROM (SELECT Department_Name AS company_name,
		   ...
		   FROM Budget_Timeline BT
            ...
            -- Since Amount 0 was included in the query, do not exclude it.
			HAVING Fiscal_Year = '2026' AND Fiscal_Quarter = '2' AND Department_Name = 'Austin Energy')t
	-- Omit the total income statement as 0.00, would default to "Default" otherwise by mistake (since not processed yet by transaction means).
    WHERE Income_Statement != 0
	-- Order the following
	ORDER BY FY ASC, FQ ASC, exp_name ASC)
    
SELECT *
FROM income_statement_expense_type_overall;
...
-- SECTION 6: Expense Transactions that are Already Processed

WITH income_statement_expense_transactions AS (
	SELECT t.company_name AS "Company Name",
		   t.FY AS "FY",
		   t.FQ AS "Quarter",
           t.exp_name AS "By Each Company-Related Expenses", 
	...
	FROM (SELECT Department_Name AS company_name,
		   ...
		   FROM Budget_Timeline BT
			...
			HAVING Fiscal_Year = '2026' AND Fiscal_Quarter = '2' AND Department_Name = 'Austin Energy')t
	-- Omit the total income statement as 0.00, would default to "Default" otherwise by mistake (since not processed yet by transaction means).
    WHERE Income_Statement != 0 AND Expense != 0
	-- Order the following
	ORDER BY FY ASC, FQ ASC, exp_name ASC)
    
SELECT *
FROM income_statement_expense_transactions;
